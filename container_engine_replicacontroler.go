...
func CreateKubeRC(name, dockerImage, external_port string, optional_args, env_args map[string]string) (string, error) {
	...
	if err != nil {
		fmt.Printf("Error creating replicacontroler named %q with error %q and stdout: %q", name, stderr.String(), stdout.String())
	}
...
func expose_rc_externally(name, external_port string) (error) {
	...
	err := expose_rc.Run()
	if err != nil {
		fmt.Printf("Error creating external load balancer on a specific port: %q, with stdout: %q", stderr.String(), stdout.String())
	}
...
func ReadKubeRC(name, external_port string) (int, string, error) {
	...
	if err != nil {
		fmt.Printf("Error listing replica controlers: %q with stdout: %q", stderr.String(), stdout.String())
	}
...
func fetchExternalIp(name string) (string, error) {
	...
	if err != nil {
		fmt.Errorf("Error fetching services information for this cluster: %q and stdout: %q", stderr.String(), stdout.String())
	}
...
func DeleteKubeRC(name, external_port string) (error) {
	...
	if err != nil {
		fmt.Printf("Error deleting replica controler: %q and stdout: %q", stderr.String(), stdout.String())
	}
